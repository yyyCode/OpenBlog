...
后端三个服务：business 与 message 需**一起构建、一起升级**（business 经 Dubbo 调 message，接口包名强耦合）；gateway 是独立入口，可单独部署。
...
CI/CD：push master 自动构建并 Docker 部署 business / message / gateway（GitHub Actions + 自托管 Runner，三条链独立，互不影响）。
